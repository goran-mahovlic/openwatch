...
Adafruit_SH1106::Adafruit_SH1106(int8_t DC, int8_t RST, int8_t CS, int8_t PWR) : Adafruit_GFX(SH1106_LCDWIDTH, SH1106_LCDHEIGHT) {
	dc = DC;
	rst = RST;
	cs = CS;
	pwr = PWR;
...
void Adafruit_SH1106::begin(void) {
	// power on
	if (pwr != -1) {
		pinMode(pwr, OUTPUT);
...
void Adafruit_SH1106::invertDisplay(const bool onOff) {
	SH1106_command(onOff ? SH1106_INVERTDISPLAY : SH1106_NORMALDISPLAY);
}

void Adafruit_SH1106::SH1106_command(const uint8_t data) { 
...
inline void Adafruit_SH1106::fastSPIwrite(const uint8_t data) {
